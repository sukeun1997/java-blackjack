...
enum Expression {
   ...
   */
        return Arrays.stream(values()).filter(e -> matchExpression(e, expression)).findFirst().orElseThrow(() ->  new IllegalArgumentException(String.format("%s는 사칙연산에 해당하지 않는 표현식입니다.", expression)));
    }
}
